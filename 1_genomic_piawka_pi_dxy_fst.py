...
def parse_piawka_dxy(filename):
	# Name output files
	fname = input_filename.split('/')[-1]
	wdir = '/'.join(input_filename.split('/')[:-1])
	if '/' in input_filename:
		wdir +=  '/'
	output_pi_filename = wdir   +  'genomic_pi_table.txt'
	output_dxy_filename1 = wdir + 'genomic_dxy_table.txt'
	output_dxy_filename2 = wdir + 'genomic_dxy_matrix.txt'
	output_fst_filename1 = wdir + 'genomic_fst_table.txt'
	output_fst_filename2 = wdir + 'genomic_fst_matrix.txt'
